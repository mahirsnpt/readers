import React from 'react';

export default function BookCard({ book, onSelect }) {
  const { title, author, rating, cover, category, description } = book;

  return (
    <div className="book-card glass-panel">
      <div className="book-cover-wrapper">
        <img 
          src={cover || "https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=500&auto=format&fit=crop&q=60"} 
          alt={`${title} Cover`}
          className="book-cover-img"
          onError={(e) => {
            e.target.onerror = null;
            e.target.src = "https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=500&auto=format&fit=crop&q=60";
          }}
        />
        <span className="book-badge">{category}</span>
      </div>
      
      <div className="book-info">
        <h3 className="book-title">{title}</h3>
        <p className="book-author">by {author}</p>
        
        <div className="book-rating">
          <svg className="book-rating-star" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
          <span>{rating.toFixed(1)}</span>
        </div>
        
        <p className="book-description">{description}</p>
        
        <div className="book-card-actions">
          <button 
            className="book-card-btn primary" 
            onClick={() => onSelect(book, 'listen')}
          >
            🎧 Listen
          </button>
          <button 
            className="book-card-btn secondary" 
            onClick={() => onSelect(book, 'read')}
          >
            📖 Read Full Summary
          </button>
        </div>
      </div>
    </div>
  );
}
